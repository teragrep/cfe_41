...
public final class CaptureStorageRequestFake implements CaptureStorageRequest {
    ...
    public CaptureStorageResponse captureStorageResponse(final int captureDefinitionId) throws IOException {
        return new CaptureStorageResponse(
                Json.createArrayBuilder().add(Json.createObjectBuilder().add("storage_id", 1).add("capture_id", captureDefinitionId).add("storage_name", "cfe_10")).add(Json.createObjectBuilder().add("storage_id", 2).add("capture_id", captureDefinitionId).add("storage_name", "cfe_11")).build()
        );
    }
}
